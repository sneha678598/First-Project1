...
def generate_predict():
    ...
    if not preprocess:
        flash('Failed to fetch weather data for the specified location.', 'error')
        return redirect(url_for('dashboard'))
    try:
    ...
    # Optionally, print to terminal for debugging
      ...
      severity_level = determine_severity(res)  # Custom function to assess severity

        # Generate alert message based on prediction
      alert_message = f"Prediction for {location}: {preprocess['keyword']} - Severity: {severity_level}"
        # Example alert levels
      alert_data = {
          "location": location,
          "prediction": preprocess['keyword'],
          "severity": severity_level,
          "message": alert_message
        }

      return render_template('dashboard.html', location=location, prediction=prediction, alert=alert_data)

    except Exception as e:
        flash(f'Error during prediction: {str(e)}', 'error')
        return redirect(url_for('dashboard'))

def determine_severity(prediction):
    """Determine the severity of a disaster based on prediction."""
    if prediction > 0.75:
        return "High"
